...
def format_input(task_load, task_eval, model):
    ...
    for ep in range(args.n_train_epochs[task_load]):
        ...
        with torch.no_grad():
            for task_eval in args.tasks:
                ...
                for n_steps, (cqs, len_cqs, _, _, _, _, _) in enumerate(test_dataloader):
                    cqs = cqs[0][0]
                    test_row = cqs.cuda().type(torch.long)
                    # Ask the algorithm to attribute our output target to input features
                    print(test_row, len(test_row), test_row.dtype)
                    attributions_ig = integrated_gradients.attribute(test_row, n_steps=200)
                    break
